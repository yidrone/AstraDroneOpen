...
void FastPlannerManager::initPlanModules(ros::NodeHandle& nh) {
  ...
  bool use_geometric_path, use_kinodynamic_path, use_topo_path, use_optimization_ros;
  ...
  nh.param("manager/use_optimization", use_optimization_ros, false);

  use_optimization_ = use_optimization_ros;  // 仅记录开关；禁用时不创建优化器
...
bool FastPlannerManager::planGlobalTraj(const Eigen::Vector3d& start_pos) {
  ...
  for (int i = 0; i < static_cast<int>(points.size()) - 1; ++i) {
    ...
    if (dist > dist_thresh) {
      int id_num = std::floor(dist / dist_thresh) + 1;
...
bool FastPlannerManager::topoReplan(bool collide) {
  ...
  if (!collide) {  // simply truncate the segment and do nothing
    ...
    if (colli_start.size() == 1 && colli_end.size() == 0) {
      ...
      for (int i = 0; i < static_cast<int>(select_paths.size()); ++i) {
        optimize_threads.emplace_back(&FastPlannerManager::optimizeTopoBspline, this, t_now,
                                      local_traj_duration, select_paths[i], i);
      }
      for (int i = 0; i < static_cast<int>(select_paths.size()); ++i) optimize_threads[i].join();
...
void FastPlannerManager::refineTraj(NonUniformBspline& best_traj, double& time_inc) {
  ...
  int             cost_function = BsplineOptimizer::NORMAL_PHASE;  // 仅用于优化分支
...
void FastPlannerManager::optimizeTopoBspline(double start_t, double duration,
  ...
  if (use_optimization_) {
    ...
    // 关闭优化：直接使用重参数化的控制点作为结果
    plan_data_.topo_traj_pos1_[traj_id] = NonUniformBspline(ctrl_pts, 3, dt);
    plan_data_.topo_traj_pos2_[traj_id] = NonUniformBspline(ctrl_pts, 3, dt);
    tm2 = tm3 = 0.0;
    ROS_INFO("optimization disabled: traj %d uses reparameterized ctrl_pts (%.3fs).", traj_id, tm1);
  }
}
...
void FastPlannerManager::planYaw(const Eigen::Vector3d& start_yaw) {
  ...
  if (duration <= 1e-6) {
    ROS_WARN("planYaw: duration too small, skip.");
    return;
  }

  double dt_yaw  = 0.3;
  int    seg_num = std::max(1, int(std::ceil(duration / dt_yaw)));
  ...
  waypts.reserve(seg_num);
  waypt_idx.reserve(seg_num);
  ...
  double te = std::max(0.0, duration - 0.1);
  Eigen::Vector3d end_v = local_data_.velocity_traj_.evaluateDeBoorT(te);
  Eigen::Vector3d end_yaw(std::atan2(end_v(1), end_v(0)), 0, 0);
  ...
  if (use_optimization_) {
    ...
    // 关闭优化：将内部控制点赋值为路径朝向
    // 控制点布局：前3个、后3个为边界，其余为内部点（索引 [3 .. seg_num-1]）
    for (int i = 0; i < static_cast<int>(waypts.size()); ++i) {
      int idx = i + 2; // 与原构造方式对齐，waypt i 对应内部控制点 idx=i+2
      if (idx >= 3 && idx < seg_num) {
        yaw(idx, 0) = waypts[i](0);  // ← 修正：取第 i 个路标的 yaw 标量
      }
    }
  }
