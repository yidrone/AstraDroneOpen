...
void OBCameraNode::onNewFrameSetCallback(std::shared_ptr<ob::FrameSet> frame_set) {
  ...
  // [Step 3] 提取 Depth 时间戳，触发 RGB 软同步
  ...
  while (enable_stream_[COLOR] && ros::ok() && is_running_.load()) {
    ...
    if (depth_frame && color_frame) {
      ...
      // 计算时间差 (Depth - Color)，单位 ms
      long long diff_us = static_cast<long long>(depth_ts_us) - static_cast<long long>(color_ts_us);
      double diff_ms = diff_us / 1000.0;

      // 打印时间差日志 (如果觉得刷屏太快，可以将 ROS_INFO_STREAM 改为 ROS_DEBUG_STREAM)
      ROS_INFO_STREAM("[Sync] Depth vs Color | Diff: " << diff_ms << " ms");
  ...
  while (enable_stream_[COLOR_LEFT] && ros::ok() && is_running_.load()) {
    ...
    auto depth_frame = frameSet->depthFrame();
    auto left_frame = frameSet->getFrame(OB_FRAME_COLOR_LEFT);

    if (depth_frame && left_frame) {
      rgb_left_is_decoded_ =
          decodeColorFrameToBuffer(left_frame, rgb_buffer_left_);

      // --- 计算时间差 ---
      uint64_t depth_ts_us = getFrameTimestampUs(depth_frame);
      uint64_t left_ts_us = getFrameTimestampUs(left_frame);
      double diff_ms = (static_cast<long long>(depth_ts_us) - static_cast<long long>(left_ts_us)) / 1000.0;

      ROS_INFO_STREAM("[Sync Left] Depth vs Left | Diff: " << diff_ms << " ms");

      auto depth_ros_time = fromUsToROSTime(depth_ts_us);
      onNewFrameCallback(left_frame, IMAGE_STREAMS.at(1), depth_ros_time);
    }
  }
  ...
  while (enable_stream_[COLOR_RIGHT] && ros::ok() && is_running_.load()) {
    ...
    auto depth_frame = frameSet->depthFrame();
    auto right_frame = frameSet->getFrame(OB_FRAME_COLOR_RIGHT);

    if (depth_frame && right_frame) {
      rgb_right_is_decoded_ =
          decodeColorFrameToBuffer(right_frame, rgb_buffer_right_);

      // --- 计算时间差 ---
      uint64_t depth_ts_us = getFrameTimestampUs(depth_frame);
      uint64_t right_ts_us = getFrameTimestampUs(right_frame);
      double diff_ms = (static_cast<long long>(depth_ts_us) - static_cast<long long>(right_ts_us)) / 1000.0;

      ROS_INFO_STREAM("[Sync Right] Depth vs Right | Diff: " << diff_ms << " ms");

      auto depth_ros_time = fromUsToROSTime(depth_ts_us);
      onNewFrameCallback(right_frame, IMAGE_STREAMS.at(2), depth_ros_time);
    }
  }
...
void OBCameraNode::onNewFrameCallback(std::shared_ptr<ob::Frame> frame,
  ...
  // ----------------- 核心修改开始 -----------------
  // 逻辑：如果传入了 forced_time (软同步时间)，就用它；否则用帧自己的时间。
  ros::Time timestamp;
  auto frame_timestamp = getFrameTimestampUs(frame); // 这一行保留，仅用于下面的 FPS 计算
  ...
  if (color_camera_info_manager_ && color_camera_info_manager_->isCalibrated() &&
    ...
    camera_info.header.stamp = timestamp; // 关键点：使用 timestamp
  ...
  image_msg->header.stamp = timestamp; // 关键点：使用 timestamp
  ...
  if (!image_flip_[stream_index]) {
    ...
    flipped_image_msg->header.stamp = timestamp; // 关键点：使用 timestamp
