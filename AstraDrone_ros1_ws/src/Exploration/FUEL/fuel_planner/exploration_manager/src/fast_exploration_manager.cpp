...
void FastExplorationManager::initialize(ros::NodeHandle& nh) {
  ...
  nh.param("exploration/planner_max_search_times", max_search_times_, 5.0);
...
int FastExplorationManager::planHomeMotion(const Vector3d& home_pos, const Vector3d& pos, const Vector3d& vel, const Vector3d& acc,
                      const Vector3d& yaw){
  // 
  ed_->views_.clear();
  ...
  std::cout << "start pos: " << pos.transpose() << ", vel: " << vel.transpose()
            << ", acc: " << acc.transpose() << ", home-pos: " << home_pos.transpose() << std::endl;
  
  double next_yaw = 0.0;
  Vector3d next_pos = home_pos;

  // ROS_WARN("planHomeMotion 1");
  ...
  planner_manager_->path_finder_->SetMaxsearchTime(max_search_times_);
  
  if (planner_manager_->path_finder_->search(pos, next_pos) != Astar::REACH_END) {
    planner_manager_->path_finder_->ResetMaxsearchTime();
...
int FastExplorationManager::planExploreMotion(
  ...
  if (planner_manager_->path_finder_->search(pos, next_pos) != Astar::REACH_END) {
    ...
    if (planner_manager_->path_finder_->search(pos, next_pos) !=
        Astar::REACH_END) {
      ROS_ERROR("No path to next viewpoint, reset Frontiers! ");
      // 25-6-5 直接找到对应的cluster， 进行特殊标记  
      frontier_finder_->ResetFrontiersToVisit(next_pos);
      planner_manager_->path_finder_->ResetMaxsearchTime();
      return FAIL;
    }
  }
