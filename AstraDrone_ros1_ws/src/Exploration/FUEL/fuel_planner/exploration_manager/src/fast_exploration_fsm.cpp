...
void FastExplorationFSM::init(ros::NodeHandle& nh) {
  ...
  nh.param("fsm/return_home", return_home_, false);
  auto_home_mode = false;

  // Auto-start exploration after takeoff
  nh.param("fsm/auto_start_explore", auto_start_explore_, true);
  nh.param("fsm/auto_start_height", auto_start_height_, 2.0);
  nh.param("fsm/auto_start_z_tol", auto_start_z_tol_, 0.15);
  auto_start_triggered_ = false;
  ...
  fd_->state_str_ = { "INIT", "WAIT_TRIGGER", "PLAN_TRAJ", "PUB_TRAJ", "EXEC_TRAJ", "FINISH", "RETURN"};
  ...
  // Listen mavros state for OFFBOARD/ARM condition
  mav_state_sub_ = nh.subscribe("/mavros/state", 10, &FastExplorationFSM::mavStateCallback, this);
  ...
  // 完成探索任务，触发保存 .pcd 地图 
  finish_pub_ = nh.advertise<std_msgs::Header>("/explore_finish", 10);

  trigger_home_sub_ =
      nh.subscribe("/explore/return_home", 1, &FastExplorationFSM::triggerHomeCallback, this);
  set_mode_client_ = nh.serviceClient<mavros_msgs::SetMode>("/mavros/set_mode");
  arming_client_   = nh.serviceClient<mavros_msgs::CommandBool>("/mavros/cmd/arming");
  extended_state_sub_ = nh.subscribe("/mavros/extended_state", 10,
                                    &FastExplorationFSM::extendedStateCallback, this);
...
void FastExplorationFSM::FSMCallback(const ros::TimerEvent& e) {
  ...
  switch (state_) {
    ...
    case WAIT_TRIGGER: {
      ...
      if (auto_start_explore_ && !auto_start_triggered_ && fd_->have_odom_) {
        const double z = fd_->odom_pos_(2);
        if (z >= auto_start_height_ - auto_start_z_tol_) {

          auto_start_triggered_ = true;

          // ⭐ 关键补丁：等价于 triggerCallback
          if (!init_pos_recorded_) {
            init_pos2return = fd_->odom_pos_;
            init_pos_recorded_ = true;

            ROS_WARN("Record Init pose for auto-returning (autoStartExplore): {%.2f, %.2f, %.2f}",
                    init_pos2return(0), init_pos2return(1), init_pos2return(2));
          }

          transitState(PLAN_TRAJ, "autoStartExplore");
        }
      }
      break;
    }
    ...
    case FINISH: {
      ...
      header_.frame_id = "uav";
      header_.seq = cc;
      header_.stamp = ros::Time::now();
      finish_pub_.publish(header_);
      cc ++;
      tryDisarmOnceIfLanded(); 
      // delay time = 0.01 * cc   500--5s
      // 探索完成后，将自动回家 
      static bool trigger_once = true; 
      if(cc > 300 && return_home_ && trigger_once && !auto_home_mode){
        trigger_once = false;
        auto_home_mode = true;
    ...
    // case RETURN:{
    //   ROS_WARN_THROTTLE(1.0, "Finish Exploration ! Auto-returning, Pls Pay Attention! ");

    //   // 当前距离比较近了， 切回Finish 
    //   if((fd_->odom_pos_ - init_pos2return).norm() < 0.5){
    ...
    case PLAN_TRAJ: {
      ...
      if(!auto_home_mode){
        ...
        // 当前距离比较近了， 切回Finish 
        if((fd_->odom_pos_ - init_pos2return).norm() < 0.3){
          triggerLandOnce();
          transitState(FINISH, "FSM");
          return;
        }else{
          // auto home mode
          int res = callHomingPlanner();
          ROS_WARN("callHomingPlanner done!");
          if (res == SUCCEED) {
            transitState(PUB_TRAJ, "FSM");
          }else{
            // Still in PLAN_TRAJ state, keep replanning
    ...
    case EXEC_TRAJ: {
      ...
      if (t_cur > fp_->replan_thresh2_ && expl_manager_->frontier_finder_->isFrontierCovered() && !auto_home_mode ) {
        transitState(PLAN_TRAJ, "FSM");
...
int FastExplorationFSM::callHomingPlanner() {
  ros::Time time_r = ros::Time::now() + ros::Duration(fp_->replan_time_);
  // ROS_WARN("expl_manager_->planHomeMotion begin!");

  int res = expl_manager_->planHomeMotion(init_pos2return, fd_->start_pt_, fd_->start_vel_, fd_->start_acc_,
                                             fd_->start_yaw_);
  classic_ = false;


  // ROS_WARN("expl_manager_->planHomeMotion done!");
  ...
  if (res == SUCCEED) {
    ROS_WARN("planHomeMotion SUCCEED!");
...
void FastExplorationFSM::clearVisMarker() {
  // visualization_->drawSpheres({}, 0.2,rqt_reconfigure Vector4d(0, 0.5, 0, 1), "points", 0, 6);
...
void FastExplorationFSM::triggerCallback(const nav_msgs::PathConstPtr& msg) {
  ...
  // record init pos 2 return 
  init_pos2return = fd_->odom_pos_;
  ROS_WARN("Record Init pose for auto-returning: {%.2f, %.2f, %.2f}", 
    init_pos2return(0), init_pos2return(1), init_pos2return(2));

}

void FastExplorationFSM::triggerHomeCallback(const std_msgs::BoolConstPtr& msg) {
  if(!msg->data || !return_home_) return;

  ROS_ERROR("Attention! triggerHomeCallback! Stop explore mission, return home: {%.2f, %.2f, %.2f}", 
    init_pos2return(0), init_pos2return(1), init_pos2return(2));
...
void FastExplorationFSM::mavStateCallback(const mavros_msgs::StateConstPtr& msg) {
  mav_state_ = *msg;
  have_mav_state_ = true;
}

void FastExplorationFSM::extendedStateCallback(
    const mavros_msgs::ExtendedStateConstPtr& msg)
{
  landed_state_ = msg->landed_state;
}

bool FastExplorationFSM::triggerLandOnce()
{
  if (land_triggered_) return true;

  mavros_msgs::SetMode sm;
  sm.request.custom_mode = "AUTO.LAND";   // PX4 常用：自动降落

  if (set_mode_client_.call(sm) && sm.response.mode_sent)
  {
    land_triggered_ = true;
    ROS_WARN("[FSM] Switched to AUTO.LAND");
    return true;
  }

  ROS_ERROR("[FSM] Failed to set mode AUTO.LAND via /mavros/set_mode");
  return false;
}

void FastExplorationFSM::tryDisarmOnceIfLanded()
{
  if (!land_triggered_ || disarm_triggered_) return;

  // PX4 landed_state: ON_GROUND 表示落地
  ...
  {
    disarm_triggered_ = true;
    ROS_WARN("[FSM] Disarmed after landing.");
  }
}
