...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  server_ = new dynamic_reconfigure::Server<plan_env::ExpBoxConfig>(mutex_);
  dynamic_reconfigure::Server<plan_env::ExpBoxConfig>::CallbackType f;
  f = boost::bind(&SDFMap::dynamicCallback, this, _1, _2);
  server_->setCallback(f);

  vis_timer_ = nh.createTimer(ros::Duration(0.5), &SDFMap::VisTimerCallback, this);
  // f_ = boost::bind(&SDFMap::dynamicCallback, this, _1, _2);
...
// 只有当参数值发生变化时才会触发回调 
void SDFMap::dynamicCallback(plan_env::ExpBoxConfig &config, uint32_t level) {

  // explore trigger 
  if(config.Start_Explore){
    geometry_msgs::PoseStamped trigger_msg;
    static ros::Publisher trigger_exp = mr_->node_.advertise<geometry_msgs::PoseStamped>("/move_base_simple/goal", 1);
    trigger_msg.header.stamp = ros::Time::now();
    trigger_msg.header.seq = 100;
    trigger_exp.publish(trigger_msg);
    
    trigger_exp.publish(trigger_msg);

    // 应用参数
    config.Start_Explore = false;
    server_->updateConfig(config);
  }

  if(config.Return_Home){
    std_msgs::Bool trigger_msg;
    static ros::Publisher trigger_home = mr_->node_.advertise<std_msgs::Bool>("/explore/return_home", 1);
    trigger_msg.data = true;
    trigger_home.publish(trigger_msg);
    // 应用参数
  ...
  bool state_change = !state_last && config.Apply_Changes;
    ...
    if (state_change) {
        ...
        explore_box_min(0) = config.BoxMin_x;
        explore_box_min(1) = config.BoxMin_y;
        explore_box_min(2) = config.BoxMin_z;

        explore_box_max(0) = config.BoxMax_x;
        explore_box_max(1) = config.BoxMax_y;
        explore_box_max(2) = config.BoxMax_z;
        ROS_ERROR(" #####################  Attention ##################### ");
        ROS_WARN(" ### Explore Box Update: min-[%.2f, %.2f, %.2f], max-[%.2f, %.2f, %.2f].", 
          explore_box_min(0), explore_box_min(1), explore_box_min(2), explore_box_max(0),
          explore_box_max(1), explore_box_max(2));

        auto gap_box = explore_box_max - explore_box_min;
        
        bool all_less = (mp_->map_min_boundary_.array() < explore_box_min.array()).all();
        bool all_greater = (mp_->map_max_boundary_.array() > explore_box_max.array()).all();


        if(!all_less || !all_greater){
          // 和 map_size 进行对比，判断是否合理... 
          ROS_WARN("Input BoxSize OUT Range of MapSize! Tuning your base params!");
        }else if(gap_box(0) > 10.0 && gap_box(1) > 10.0 && gap_box(2) > 2){
          mp_->box_mind_ = explore_box_min;
          mp_->box_maxd_ = explore_box_max;
          posToIndex(mp_->box_mind_, mp_->box_min_);
          posToIndex(mp_->box_maxd_, mp_->box_max_);
          ROS_WARN(" Apply new Explore Box! ");
        }else{
          ROS_WARN("Check your Box setting! Box-max need bigger than Box-min! ");
        }

        ROS_ERROR(" #####################  Attention ##################### ");
    }else{
      // 临时的探索区域box  可视化
      temp_box_min(0) = config.BoxMin_x;
      temp_box_min(1) = config.BoxMin_y;
      temp_box_min(2) = config.BoxMin_z;

      temp_box_max(0) = config.BoxMax_x;
      temp_box_max(1) = config.BoxMax_y;
      temp_box_max(2) = config.BoxMax_z;
    }

    state_last = config.Apply_Changes;
}

void SDFMap::VisTimerCallback(const ros::TimerEvent &event){
  Eigen::Vector4d color_exp{1.0, 0, 0, 1.0};
  visualizeBoundingBox(mp_->box_mind_, mp_->box_maxd_, color_exp, 0, 0.8);

  Eigen::Vector4d color_temp{0, 0, 1.0, 0.80};
  visualizeBoundingBox(temp_box_min, temp_box_max, color_temp, 1);
}

// visual box 
void SDFMap::visualizeBoundingBox(const Eigen::Vector3d& box_min,
  const Eigen::Vector3d& box_max, const Eigen::Vector4d& color, int id, double size_x) {
  // double size_x = 0.4;
  ...
  static ros::Publisher bbox_pub = mr_->node_.advertise<visualization_msgs::MarkerArray>("/explore_box/update_bbox", 1);
