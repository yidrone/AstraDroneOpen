...
#include <plan_env/ExpBoxConfig.h>  
...
private:
  ...
  // 动态参数服务器
  void dynamicCallback(plan_env::ExpBoxConfig &config, uint32_t level);
  boost::recursive_mutex mutex_;
  dynamic_reconfigure::Server<plan_env::ExpBoxConfig>* server_;
  // 动态调整的探索区域
  Eigen::Vector3d explore_box_min, explore_box_max;

  Eigen::Vector3d temp_box_min, temp_box_max;

  // visual box 
  void visualizeBoundingBox(const Eigen::Vector3d& box_min,
    const Eigen::Vector3d& box_max, const Eigen::Vector4d& color, int id, double size_x = 0.4);
