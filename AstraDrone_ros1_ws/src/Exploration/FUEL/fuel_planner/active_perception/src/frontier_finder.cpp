...
FrontierFinder::FrontierFinder(const EDTEnvironment::Ptr& edt, ros::NodeHandle& nh) {
  ...
  // 使用雷达的可视性检查
  nh.param("frontier/mid360_fov", lidar_.use_lidar_model, false);
  // 倾斜安装角度 
  nh.param("frontier/lidar_pitch", lidar_.fixed_pitch, 0.0);

  if(lidar_.use_lidar_model){
    double degree2rad = 0.01745329252; 
    lidar_.vertial_min = -7.0 * degree2rad;
    lidar_.vertial_max = 52.0 * degree2rad;
    lidar_.horizon_360 = true;
  }

  // 雷达的感知半径 
  nh.param("sdf_map/max_ray_length", lidar_.sensing_max, 15.0);
  // 滤除地面无效前沿聚类
  nh.param("frontier/filter_ground_ftr", filter_ground_ftr, false);
  ROS_WARN(" filter_ground_ftr STATE: %d", filter_ground_ftr);
...
// 移除最佳视点不可达的frontier 
void FrontierFinder::ResetFrontiersToVisit(Eigen::Vector3d best_vp) {
  // 统一检测一下... 滤除不合适的frontier 
  list<Frontier> frontiers_filter;
  for (auto& ft : frontiers_) {
    if(ft.viewpoints_.empty()){
      dormant_frontiers_.push_back(ft);
      continue;
    }
    double dis_gap = (best_vp-ft.viewpoints_.front().pos_).norm();
    if( dis_gap < 1.0){
      // dormant_frontiers_.push_back(ft);

      // 这里存在系列互相的代价，不能直接 删除，只能标记状态 
      ft.invalid_ftr = true;

      continue;
    }
    // frontiers_filter.emplace_back(ft);
  }

  // frontiers_.swap(frontiers_filter);
...
void FrontierFinder::computeFrontierInfo(Frontier& ftr) {
  ...
  // ######## 基于法向量滤除无价值前沿聚类 ##########
  ftr.invalid_ftr = false;
  if(filter_ground_ftr){
    Eigen::Vector3d _normal_, average_norm_;
    for(auto cell: ftr.filtered_cells_){
      _normal_.setZero(); 
      frontierNormal(cell, _normal_);
      average_norm_ += _normal_;
    }
    average_norm_ /= double(ftr.filtered_cells_.size());
    // 计算前沿点的法向量和Z轴正方向向量之间的最小夹角
    double dot_product1 = average_norm_.dot(Eigen::Vector3d{0,0,1});
    // 防止浮点误差导致cos值超出[-1,1]范围
    dot_product1 = std::max(-1.0, std::min(1.0, dot_product1));
    // 计算夹角（弧度制）  0~pi
    double normal_ang2Zaxis = std::acos(dot_product1); // 计算夹角（弧度制）

    // 计算前沿点的法向量和Z轴正方向向量之间的最小夹角
    double dot_product2 = average_norm_.dot(Eigen::Vector3d{0,0,-1});
    // 防止浮点误差导致cos值超出[-1,1]范围
    dot_product2 = std::max(-1.0, std::min(1.0, dot_product2));
    // 计算夹角（弧度制）  0~pi
    double normal_ang2Zaxis2 = std::acos(dot_product2); // 计算夹角（弧度制）

    // double gap_ = 30*0.01745329252; 
    double gap_ = 45*0.01745329252; 
    if( normal_ang2Zaxis<gap_ || normal_ang2Zaxis2<gap_ ){
      ftr.invalid_ftr = true;
    }
  }

}

// return normal for frontier_cell 
inline void FrontierFinder::frontierNormal(const Eigen::Vector3d& cell, Vector3d& normal){
  Eigen::Vector3i pose, tmp;  // temp_idx 
  edt_env_->sdf_map_->posToIndex(cell, pose);
  Vector3i normal_dir_{0, 0, 0};
  for (auto nbr_ : nbr_dir){
    tmp = pose + nbr_;
    // At least one neighbor is unknown
    if (edt_env_->sdf_map_->getOccupancy(tmp) == SDFMap::UNKNOWN){
      // normal_dir_ += nbr_;
      normal_dir_ -= nbr_;
    }
  }
  normal(0) = double(normal_dir_(0));
  normal(1) = double(normal_dir_(1));
  normal(2) = double(normal_dir_(2));
...
void FrontierFinder::getTopViewpointsInfo(
  ...
  for (auto frontier : frontiers_) {
    // 无效ftr skip 
    if(frontier.invalid_ftr) continue;
  ...
  for (double rc = candidate_rmin_, dr = (candidate_rmax_ - candidate_rmin_) / candidate_rnum_;
    ...
    for (double phi = -M_PI; phi < M_PI; phi += candidate_dphi_) {
      ...
      if(!lidar_.use_lidar_model)
        ...
        visib_num = CheckVisibleLidar(sample_pos, avg_yaw, cells);
      }
...
// 基于雷达模型的可视性检测  方位角没有用
int FrontierFinder::CheckVisibleLidar(const Vector3d& pos, const double& yaw, const vector<Vector3d>& cluster){
  int visib_num = 0;
  Eigen::Vector3i idx;

  // 雷达安装存在pitch角度  lidar_.fixed_pitch
  // 旋转矩阵 
  Eigen::Isometry3f temp_transform_world2lidar;
  temp_transform_world2lidar = Eigen::Isometry3f::Identity();
  temp_transform_world2lidar.translate(pos.cast<float>());
  // ZYX 顺序：yaw(Z), pitch(Y), roll(X)
  temp_transform_world2lidar.rotate(
    Eigen::AngleAxisf(yaw, Eigen::Vector3f::UnitZ()) *
    // Eigen::AngleAxisf(0.0, Eigen::Vector3f::UnitY()) *  // add pitch 
    Eigen::AngleAxisf(lidar_.fixed_pitch, Eigen::Vector3f::UnitY()) *
    Eigen::AngleAxisf(0.0, Eigen::Vector3f::UnitX())
    );
  temp_transform_world2lidar = temp_transform_world2lidar.inverse();
  

  for (auto cell : cluster) {
    // // Check if frontier cell is inside FOV
    // if(!lidarModel_.CheckPointInside(pos, cell)) continue;

    // 是否在感知范围内... 
    if( (pos-cell).norm()> lidar_.sensing_max*0.9 )
      continue;

    // 待观测的前沿单元 转换到雷达系下 
    Eigen::Vector3f cell_lidar_frame_ =
        temp_transform_world2lidar * cell.cast<float>();

    // 获取垂直角（pitch）：z 与水平面的夹角（rad）
    // float pitch = std::atan2(pt.z, std::sqrt(pt.x * pt.x + pt.y * pt.y));
    float pitch = std::atan2(cell_lidar_frame_(2), std::sqrt(cell_lidar_frame_(0) * cell_lidar_frame_(0) 
      + cell_lidar_frame_(1) * cell_lidar_frame_(1)));

    // 不在视野范围内，skip 
    if(pitch < -lidar_.vertial_min || pitch > lidar_.vertial_max)
        continue;
