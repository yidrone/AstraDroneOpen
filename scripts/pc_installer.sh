...
SCRIPTS=(
  ...
  "./env_sh/01_env_px4_init_pc.sh"
  "./env_sh/02_env_third_party_init.sh"
  "build_sim_workspace.sh"
  "build_AstraDrone_ros1.sh"   # 会自动附加 --small_build
)
...
# ============ 网络检查提示 ============
echo_warn "本脚本后续操作需要正常连接到 GitHub, 请仔细检查网络设置, 且需要一个尽量干净的环境!!"
echo_warn "本脚本耗时较长(> 20min)，请注意中途的输入密码提示!"
sleep 3  # 停留3秒让用户看到提示
...
run_script() {
  ...
  # 针对 build_AstraDrone_ros1.* 自动附加最小编译参数
  case "$(basename "$path")" in
    build_AstraDrone_ros1.sh|build_AstraDrone_ros1.bin)
      args+=(--small_build clean_build)
      ;;
  esac
...
echo "~/AstraDrone/scripts/run_sh/pc_example.sh"
